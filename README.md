...
### 🚀 Spring AOP 감사 로그 도입 및 유저 정지(Ban) 기초 설계 (v1.5)

프로젝트의 보안성과 추적성을 글로벌 서비스 수준으로 끌어올리기 위해 **'AOP 기반 감사 로그'**와 **'유저 정지(Ban) 관리 아키텍처'**의 코어 로직을 완벽하게 병합했습니다.
...
#### 2. 🚫 유저 정지(Ban) 시스템 정책 및 세션 만료 구축
- **도메인 확장:** `User` 엔티티에 정지 여부를 판별할 `isBanned` 필드와 상태 변경 메서드를 추가했습니다.
- **일반 로그인 차단:** `UserService.login` 시점에 정지된 회원일 경우 즉각 예외를 던져 진입을 차단합니다.
...
- **토큰 재발급 방어벽:** `RefreshTokenService`에서 갱신(Refresh) 요청이 올 때 해당 유저가 정지 상태인지 2차 검증하여 세션 연장을 원천 봉쇄합니다.
...
* **해결:** 윈도우 운영체제(OS) 명령어를 활용해 배후의 좀비 프로세스를 직접 색출하고 강제 종료했습니다.
  1. `netstat -ano | findstr :8080` 명령어로 포트를 웅켜쥐고 있는 프로세스 ID(PID) 확인.
