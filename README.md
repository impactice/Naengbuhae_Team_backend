...
### 🚨 [기능 구현] 식약처 회수/판매중지 식품 공공 API 연동 및 스케줄러 자동화 (v1.7)

사용자의 냉장고에 보관된 식재료가 식중독균 검출 등으로 회수 조치되었을 경우, 이를 즉각 감지하고 경고하기 위해 **식약처 공공데이터(OpenAPI)** 연동 인프라를 선제적으로 구축했습니다.

#### 1. ⏱️ Spring Scheduler 기반 자동화 파이프라인
- **개요:** 관리자가 수동으로 데이터를 갱신할 필요 없이, 서버가 스스로 식약처 데이터를 감시하도록 백그라운드 자동화 심장을 달았습니다.
- **적용:** `@EnableScheduling` 및 `@Scheduled` 어노테이션을 활용하여 주기적으로 최신 위해 식품 데이터를 호출하는 `FoodSafetyScheduler`를 구현했습니다.
- **보안 격리:** 식약처 API 인증키는 깃허브 유출을 원천 봉쇄하기 위해 `.env` 환경변수(`FOOD_SAFETY_API_KEY`)로 완벽하게 격리하고, `application.properties`를 거쳐 안전하게 주입되도록 설계했습니다.

---

## 🛠️ 트러블슈팅: 공공 API 한글 인코딩 깨짐(Mojibake) 완벽 방어

식약처 공공 API 연동 과정에서 발생한 고질적인 텍스트 인코딩 충돌 문제를 추적하고 해결하여 데이터 무결성을 확보한 기록입니다.

* **증상:** `RestTemplate`을 통해 식약처 API 응답을 일반 `String` 타입으로 파싱할 때, 스프링 부트 서버와 API 서버 간의 인코딩 규격 불일치로 인해 한글 데이터("맥반석 버터구이오징어" -> "留λ 踰곌뎄댁ㅼ")가 심각하게 깨지는 현상 발생.
* **1차 시도:** `RestTemplate`에 `StringHttpMessageConverter(StandardCharsets.UTF_8)`를 강제 주입하여 해결을 도모했으나, 식약처 서버 측에서 응답 헤더(Header)에 명확한 인코딩 포맷을 제공하지 않아 문자열 강제 변환 방식으로는 복구 불가 판정.
* **최종 해결 (Jackson 파싱 고도화):** - 응답 데이터를 `String` 텍스트로 임시 저장하는 과정을 아예 생략하고, Jackson 라이브러리의 **`JsonNode.class` 객체로 즉시 바인딩(역직렬화)** 하도록 아키텍처를 전면 수정했습니다.
  - 이를 통해 HTTP 응답 바이트를 Jackson 모듈이 직접 분석하여 최적의 UTF-8 한글로 자동 복구하게 만들었으며, 수동 컨버터 설정 없이도 100% 완벽한 데이터 무결성(Data Integrity)을 확보했습니다.

### 🕵️‍♂️ [기능 구현] 공공데이터(JSON) 정밀 파싱 및 핵심 데이터 추출 (v1.8)

식약처 API로부터 수신한 대용량 JSON 데이터에서, 우리 서비스의 비즈니스 로직(위험 식재료 매칭)에 반드시 필요한 핵심 정보만을 선별하여 추출하는 정밀 파싱(Parsing) 로직을 구축했습니다.

#### 1. Jackson 라이브러리 기반 안전한 역직렬화
- **적용:** 응답받은 데이터 뭉치를 단순 문자열(String)이 아닌 `JsonNode` 객체로 즉시 맵핑하여 데이터의 구조적 무결성을 확보했습니다.
- **트리 탐색:** 식약처 데이터 고유의 중첩된 JSON 구조(`I0490` -> `row` 배열)를 `path()` 메서드로 안전하게 파고들어 타겟 배열을 확보했습니다.

#### 2. 핵심 비즈니스 데이터 선별 (Data Filtering)
- **추출 타겟:** 수많은 반환 필드 중 사용자의 냉장고 데이터와 직접 대조할 **1) 제품명(`PRDTNM`)**, 경고 메시지에 포함될 **2) 제조업체명(`BSSHNM`)**, **3) 회수사유(`RTRVLPRVNS`)** 단 3가지만을 핀포인트로 추출(asText)하여 메모리 낭비를 최소화했습니다.
- **로그 모니터링 연동:** 추출된 위험 식품의 리스트를 `[위험식품 감지] 제품명: ~, 업체명: ~, 사유: ~` 형태로 서버 콘솔에 실시간 로깅하여 시스템 관리자의 모니터링 편의성을 높였습니다.

### 🚨 [비즈니스 로직] 위험 식품 DB 자동 스캔 및 감지 알고리즘 (v1.9)

식약처 공공 API에서 추출한 위해 식품 데이터를 바탕으로, 실제 사용자의 냉장고 데이터베이스를 실시간으로 스캔하여 위험 요소를 찾아내는 핵심 비즈니스 로직을 완성했습니다.

#### 1. 부분 일치(Partial Match) 기반 스캔 로직 구현
- **JPA 쿼리 고도화:** `IngredientRepository`에 `findByNameContaining(String keyword)` 쿼리 메서드를 도입하여, 식약처에서 반환한 구체적인 제품명이 사용자가 등록한 식재료 이름의 일부로 포함되어 있는지(LIKE 검색) 추적하는 스캔 엔진을 구축했습니다.
- **아키텍처 통합:** `FoodSafetyScheduler`에 `IngredientRepository`를 의존성 주입(DI)하여, 데이터 수집 파이프라인과 내부 DB 스캔 로직을 완벽하게 하나의 트랜잭션 흐름으로 통합했습니다.

#### 2. 선제적 위험 감지 및 모니터링 시스템
- 스케줄러가 백그라운드에서 동작하며 위험 식품 매칭에 성공할 경우, 즉각적으로 해당 식재료의 소유자(User)를 식별하고, 서버 콘솔에 `[초긴급]` 플래그를 동반한 상세 경고 로그(제품명, 사유, 유저 정보)를 출력하여 관리자의 가시성을 극대화했습니다.

### 🚨 FCM 실시간 긴급 푸시 알림 및 가족 공유 안전망 구축 (v2.0)

식약처 데이터를 통해 위험 식품이 감지되었을 때, 이를 단순히 서버 로그에만 남기는 것이 아니라 **Firebase Cloud Messaging (FCM)**을 통해 사용자의 스마트폰으로 즉시 경고를 전송하는 실시간 안전망을 최종 완성했습니다.

#### 1. 공유 냉장고 '가족 전원' 알림 전파 아키텍처
- **문제 해결:** 식재료를 등록한 '작성자 1인'에게만 알림이 가는 논리적 결함을 발견하여 즉각 리팩토링했습니다.
- **적용:** 식재료 엔티티에서 해당 식재료가 보관된 `Fridge` 객체를 역추적하고, `FridgeMemberRepository`를 통해 해당 냉장고를 공유하는 **모든 구성원(가족)의 명단을 추출하여 전원에게 푸시 알림을 브로드캐스팅**하도록 로직을 고도화했습니다. (어머니가 등록한 위험 식품을 자녀도 즉시 인지 가능)

#### 2. 사용자 경험(UX)을 고려한 멀티 디바이스 대응
- 유저가 사용하는 모든 기기(스마트폰, 태블릿 등)의 토큰을 조회하여 알림을 동시 발송하는 `FcmService`의 기능을 적극 활용했습니다.
- **"보관 중인 'OOO'이(가) 식약처 회수 대상(사유)으로 지정되었습니다. 절대 섭취하지 마세요!"**라는 구체적이고 명확한 메세지를 조립하여 사용자의 즉각적인 조치를 유도합니다. 이로써 단순 재료 관리를 넘어 '사용자의 건강을 책임지는 지능형 스마트 시스템'이 완성되었습니다.

### 🚀 [성능 최적화 및 아키텍처 고도화] 상위 1% 백엔드 내실 다지기 (v2.1)

단순한 기능 구현을 넘어, 대규모 트래픽과 글로벌 클라우드 운영 환경을 고려한 엔터프라이즈급 서버 최적화 및 리팩토링을 수행했습니다.

#### 1. 객체지향적 설계: Enum 도입으로 타입 안정성(Type Safety) 확보
- **문제:** 기존 `String`과 정규식(`@Pattern`)에 의존하던 프로필 상태 값(성별, 활동량, 식단 목표)은 런타임 오타나 비즈니스 로직 오류에 취약했습니다.
- **해결:** `Gender`, `ActivityLevel`, `DietGoal`을 강력한 `Enum` 객체로 전면 전환하여 컴파일 타임에 무결성을 보장하도록 개선했습니다.
- **디테일:** `@JsonValue` 어노테이션을 적용하여 백엔드 내부에서는 객체지향적인 Enum을 사용하되, 프론트엔드 API 통신 시에는 기존 한글 문자열 스펙을 그대로 유지하여 프론트엔드의 수정 소요를 0건으로 만들었습니다.
- **트러블슈팅:** Enum 마이그레이션 과정에서 발생한 기존 DB 텍스트 데이터와의 직렬화 충돌(`No enum constant`) 문제를 파악하고, `ddl-auto=create` 옵션으로 스키마 및 더미 데이터를 안전하게 초기화하여 해결했습니다.

#### 2. 이메일 발송 비동기화(@Async)를 통한 DB 커넥션 풀 보호
- **문제:** 회원가입 트랜잭션 내에서 이메일을 발송할 경우, 외부 네트워크 지연(1~2초) 동안 DB 커넥션을 반납하지 못해 트래픽 병목의 원인이 됩니다.
- **해결:** `MailService`의 발송 로직에 `@Async`를 적용하여 별도의 워커 스레드(Worker Thread)로 분리했습니다.
- **효과:** 메인 스레드는 DB 커넥션을 즉시 반납하게 되어, 트래픽 폭주 시에도 서버의 고가용성과 응답 속도를 완벽하게 유지할 수 있습니다.

#### 3. 글로벌 클라우드 배포를 대비한 타임존(Timezone) 고정
- **해결:** 시스템 내 모든 스케줄러(`FoodSafetyScheduler`, `RefreshTokenCleanupScheduler` 등)의 `@Scheduled` 어노테이션에 `zone = "Asia/Seoul"` 속성을 명시했습니다.
- **효과:** 추후 AWS, GCP 등 기본 시간이 UTC로 설정된 클라우드 인프라에 배포하더라도, 서버 운영체제 시간에 종속되지 않고 한국 시간(KST) 새벽에 정확히 배치 작업이 실행되도록 무결성을 확보했습니다.

### 🤖 [핵심 기능] AI 레시피 추천 시스템 실전 연동 및 안정성 고도화 (v2.2)

사용자의 냉장고 데이터를 기반으로 맞춤형 요리를 제안하는 '스마트 냉부해'의 핵심 AI 비서 로직을 완벽하게 개통했습니다.

#### 1. 공유 냉장고 기반 지능형 데이터 수집
- 단순히 사용자 개인의 식재료가 아닌, **해당 사용자가 속한 공유 냉장고 전체의 식재료**를 DB에서 효율적으로 스캔(`IngredientRepository`)하여 AI 서버로 전달하도록 로직을 고도화했습니다.
- 외부 AI 서버(FastAPI)의 JSON 스펙(`{ "ingredients": [...] }`)에 맞춰 데이터를 가공하여 API 통신의 무결성을 확보했습니다.

#### 2. 엔터프라이즈급 외부 API 통신 안정성 (Fallback)
- `RestTemplate`에 명시적인 타임아웃(Connect 5초, Read 30초)을 설정하여, 외부 AI 서버의 응답 지연이 메인 서버의 스레드 고갈(장애 전파)로 이어지지 않도록 방어막을 구축했습니다.
- API 호출 실패나 예외 발생 시 서버가 다운되지 않도록 예외를 격리(Catch)하고, 프론트엔드에 안전한 빈 객체(Fallback DTO)를 반환하도록 시니어급 에러 핸들링을 적용했습니다.

### 🛡️ [보안 및 로직 고도화] 무결점 방어 아키텍처 및 권한 제어 구축 (v2.3)

단순한 기능 구현을 넘어, 엔터프라이즈 환경에서 발생할 수 있는 치명적인 논리적 버그와 악의적인 공격(Hacking) 시나리오를 선제적으로 차단했습니다.

#### 1. 레시피 추천 엔진 고도화 및 IDOR 보안 취약점 차단
- **기능 개선:** 사용자의 개인 식재료만 스캔하던 기존 레시피 추천 로직을, **"사용자가 속한 가족 공유 냉장고 전체"**를 스캔하도록 상향 평준화하여 기획 의도에 완벽하게 부합시켰습니다.
- **보안 패치 (IDOR 방어):** 타인의 냉장고 ID를 파라미터로 넘겨 데이터를 탈취하는 부적절한 직접 객체 참조(IDOR) 공격을 방어하기 위해, `FridgeMemberRepository`를 통한 강력한 접근 권한(Authorization) 검증 로직을 추가했습니다.

#### 2. Brute-Force(사전 대입) 공격 방어를 위한 지능형 계정 잠금 시스템
- **보안 메커니즘:** 무차별적인 비밀번호 대입 공격을 차단하기 위해 `User` 엔티티에 실패 카운트(`failedAttempt`)와 잠금 시간(`lockTime`)을 캡슐화하고, 5회 연속 실패 시 계정을 15분간 강제 잠금(Lock)하는 보안망을 구축했습니다.
- **트랜잭션 정밀 제어 (AOP):** Spring의 트랜잭션 롤백 정책으로 인해 로그인 실패 시 예외가 발생하면 실패 기록까지 롤백되는 프레임워크의 맹점을 파악하고, `@Transactional(noRollbackFor = ...)` 옵션을 선언하여 보안 감사의 원자성(Atomicity)을 완벽하게 보장했습니다.

### ⚡ [성능 최적화] 데이터베이스 N+1 쿼리 병목 해결 및 조회 성능 개선 (v2.4)

서비스 규모 확장에 대비하여, 레시피 추천 및 목록 조회 시 발생하는 치명적인 데이터베이스 성능 병목 현상을 선제적으로 해결했습니다.

#### 1. JPA 지연 로딩(Lazy Loading)으로 인한 N+1 문제 진단
- **문제:** 기존 로직은 레시피 목록을 조회한 후, 각 레시피와 연관된 작성자(`User`) 및 식재료(`Ingredient`) 정보를 지연 로딩으로 가져왔습니다. 이로 인해 레시피가 N개일 경우 1+N번의 추가 쿼리가 발생하는 성능 저하 요인을 발견했습니다.

#### 2. @EntityGraph를 활용한 한 방 쿼리(Fetch Join) 최적화
- **해결:** `RecipeRepository`에 `@EntityGraph(attributePaths = {"user", "ingredients"})` 어노테이션을 적용한 `findAllOptimized()` 메서드를 신규 도입했습니다.
- **효과:** 복잡한 JPQL을 직접 작성하는 대신 Spring Data JPA의 기능을 적극 활용하여, 단 한 번의 조인(JOIN) 쿼리만으로 연관된 모든 엔티티를 즉시 로딩(Eager Fetch)하도록 개선했습니다. 이를 통해 대량의 레시피 조회 시 DB 부하를 획기적으로 낮추고 응답 속도를 극대화했습니다.

### 🛡️ [안정성 및 성능] 프로덕션 레벨 예외 처리 및 대용량 트래픽 대비 최적화 (v2.5)

실제 서비스 운영 환경에서 발생할 수 있는 자원 고갈, 데이터베이스 병목, 그리고 세션 탈취 시나리오를 방어하기 위한 3단계 핵심 최적화를 수행했습니다.

#### 1. 외부 API 의존성 격리 (Gemini API 타임아웃 적용)
- **문제:** 이미지 인식을 담당하는 구글 Gemini API 호출 시 타임아웃이 설정되어 있지 않아, 외부 서버 지연 시 내부 톰캣(Tomcat) 스레드가 무한정 블로킹되는 치명적 결함을 식별했습니다.
- **해결:** `RestTemplateBuilder`를 통해 `Connect Timeout(5초)`, `Read Timeout(60초)`을 명시적으로 강제하여, 외부 시스템의 장애가 우리 서버 전체의 마비로 이어지는 '장애 전파(Cascading Failure)'를 원천 차단했습니다.

#### 2. 대량 데이터 삭제 쿼리 최적화 (Bulk Operation)
- **문제:** 여러 개의 식재료를 일괄 삭제할 때 JPA의 엔티티 개수만큼 `DELETE` 쿼리가 발생하는 N+1 쿼리 병목을 발견했습니다.
- **해결:** `ingredientRepository.deleteAllInBatch(targets)`를 도입하여 루프 삭제 방식을 단 1번의 벌크(Bulk) 삭제 쿼리로 최적화하여 DB 부하를 획기적으로 낮췄습니다. (삭제 이력은 로깅 서비스로 안전하게 분리)

#### 3. 보안 무결성: 비밀번호 변경 시 세션 강제 만료 (Session Revocation)
- **보안 패치:** 계정 탈취 상황을 가정하여, 사용자가 비밀번호를 변경하는 즉시 `RefreshTokenService`를 호출해 다른 모든 기기에 로그인되어 있던 리프레시 토큰을 강제 폐기(Revoke)하도록 로직을 보완했습니다.

### 🤖 [버그 수정 및 아키텍처 고도화] AI 레시피 추천 통신 규격 정상화 및 성능 최적화 (v2.6)

백엔드와 외부 AI 서버(FastAPI) 간의 데이터 규격 불일치를 해결하고, 서비스 전반의 데이터베이스 병목 현상을 선제적으로 튜닝했습니다.

- **문제 진단:** AI 서버가 요구하는 복합 데이터(User ID, 취향, 재료 리스트)와 백엔드의 송신 규격이 일치하지 않아 422 에러가 발생하는 연동 결함을 확인했습니다.
- **해결 및 고도화:** AI 서버의 Pydantic 모델에 완벽히 대응하는 `AiRecipeRequestDto` 및 `AiRecipeListResponseDto`를 신규 설계했습니다. 특히, DB에 저장된 사용자의 다이어트 목표(`user_preference`)를 AI 프롬프트에 동적으로 주입하여, 단순 기능 연동을 넘어 기획 의도에 부합하는 초개인화 추천 시스템을 완성했습니다.

- **[성능] JPA 컬렉션 페치(Fetch) 한계 돌파 및 N+1 완전 박멸:**
    - **문제:** 냉장고 목록이나 레시피 목록 조회 시, 연관된 자식 컬렉션(`ingredients`, `members` 등)을 로드하기 위해 1+N번의 쿼리가 폭주하는 현상을 진단했습니다.
    - **해결 방안:** 무분별한 `JOIN FETCH` 사용 시 발생할 수 있는 `MultipleBagFetchException`(메모리 곱집합 폭발) 위험을 방지하기 위해, 하이버네이트의 `default_batch_fetch_size` 최적화 기법을 글로벌하게 적용했습니다.
    - **효과:** N+1 쿼리 폭주를 단일 `IN` 쿼리로 묶어 처리(Batch Fetching)함으로써, 대규모 데이터 조회 시 쿼리 수를 최대 1/100 수준으로 획기적으로 줄이고 서버 응답 속도와 메모리 안정성을 동시에 확보했습니다.
 
- **[성능/안정성] JPA MultipleBagFetchException 및 메모리 폭발 원천 차단:**
    - **문제:** N+1 문제 해결을 위해 2개 이상의 자식 컬렉션(`ingredients`, `steps`)을 동시에 `JOIN FETCH` 할 경우, Cartesian Product(곱집합)가 발생하여 심각한 메모리 팽창 및 서버 마비 위험성을 진단했습니다.
    - **해결:** 컬렉션에 대한 무분별한 Fetch Join을 과감히 제거하고 단일 연관 객체(`User`)에만 제한적으로 적용했습니다. 대신 글로벌 설정인 `default_batch_fetch_size`를 도입하여, 지연 로딩 시 발생하는 다량의 단건 쿼리를 `IN` 쿼리로 병합(Batching) 호출하도록 구조를 재설계했습니다. 이를 통해 N+1 성능 병목 해소와 메모리 안정성이라는 두 마리 토끼를 모두 잡았습니다.

- **[아키텍처/안정성] GET 요청의 멱등성(Idempotency) 확보 및 부수 효과 제거:**
    - **문제:** 식재료 목록 조회(GET) 요청 시, 사용자의 냉장고가 존재하지 않으면 백그라운드에서 강제로 냉장고를 생성(INSERT)하는 논리적 결함(Side-effect)을 발견했습니다. 이는 REST API 설계 원칙에 위배되며 트랜잭션 에러를 유발할 수 있습니다.
    - **해결:** 조회 로직과 생성 로직을 철저히 분리하고, 냉장고가 없을 시 명확한 예외(`IllegalArgumentException`)를 던져 클라이언트 측에서 정상적인 생성 플로우를 타도록 아키텍처를 교정했습니다.

- **[보안] JWT 토큰 권한 검증(Authorization) 강제화 및 취약점 제거:**
    - **문제:** 조작된 JWT 토큰이나 권한 파싱 실패 시, 서비스 중단을 막기 위해 임의로 최하위 권한(USER)을 부여하고 통과시키는 무서운 보안 백도어(Backdoor) 코드를 식별했습니다.
    - **해결:** 권한 검증에 실패할 경우 가차 없이 `RuntimeException`을 던져 스프링 시큐리티 필터 단에서 `401 Unauthorized`로 접근을 원천 차단하도록 보안 정책을 엄격하게 상향시켰습니다.

### 🚀 [성능 및 데이터 정합성] 도메인 전역의 계정 탈퇴 로직 최적화 및 캐시 무결성 확보 (v2.7)

식재료 도메인에 제한적으로 적용했던 벌크 연산(v2.5)을 서비스 전역(회원 탈퇴 프로세스)으로 확장하고, 대규모 삭제 시 발생하는 메모리 불일치 위험을 완벽하게 차단했습니다.

* **⚡ 참조 무결성(FK) 기반의 전역 벌크 삭제(Bulk Delete) 파이프라인 구축:**
    * **문제:** 사용자 계정 탈퇴 시, Spring Data JPA의 영속성 전이(Cascade) 및 기본 삭제 메서드가 수천 개의 연관 엔티티(냉장고, 레시피, 알림 등 8개 도메인)를 건건이 `SELECT-DELETE` 하여 DB 커넥션을 고갈시키는 문제를 진단했습니다.
    * **해결:** 자식 엔티티부터 부모 엔티티까지 외래키 제약 조건을 고려한 정밀한 순서로 `deleteAll...InBatch` 커스텀 쿼리를 전수 도입하여, 탈퇴 트랜잭션의 소요 시간과 DB 락(Lock) 점유 시간을 극단적으로 단축했습니다.
* **🛡️ 벌크 연산의 역효과(1차 캐시 오염) 원천 차단:**
    * **문제:** 벌크 연산이 JPA의 1차 캐시(영속성 컨텍스트)를 우회하여 DB에 직행함에 따라, 서버 메모리에는 삭제된 데이터가 남아있는 '데이터 불일치(Phantom Data)' 위험성을 식별했습니다.
    * **해결:** 모든 벌크 연산 쿼리에 `@Modifying(clearAutomatically = true)` 옵션을 강제하여 연산 직후 영속성 컨텍스트를 즉각 동기화(초기화)함으로써, 런타임 환경에서의 완벽한 데이터 정합성(Consistency)을 보장했습니다.

### 🛡️ [보안 고도화] ID 열거 공격(IDOR) 원천 차단 및 시스템 방어막 구축 (v2.8)

악의적인 사용자가 식별자를 추측하여 타인의 데이터에 무단 접근하는 취약점을 방어하기 위해, 핵심 도메인의 데이터베이스 구조와 에러 핸들링 로직을 전면 개편했습니다.

* **🔒 예측 불가능한 식별자(UUID) 마이그레이션:**
    * **문제:** 핵심 엔티티인 냉장고(`Fridge`)의 Primary Key가 순차적으로 증가하는 `Long` 타입으로 설계되어 있어, 봇(Bot)을 이용한 무차별 대입 공격(ID Enumeration)에 취약함을 진단했습니다.
    * **해결:** `Fridge` 및 연관된 4개의 하위 엔티티의 식별자를 36자리 난수인 `UUID` 체계로 전면 마이그레이션하여, 타인의 자원 ID를 유추할 수 있는 확률을 수학적으로 차단했습니다.
* **🛡️ 기술 스택 노출 방지 (Global Exception Handling):**
    * **문제:** 악의적인 문자열 입력 시 발생하는 스프링 내부 예외(`MethodArgumentTypeMismatchException`)가 그대로 노출되어 프레임워크 기술 스택이 탄로 날 위험성을 식별했습니다.
    * **해결:** `GlobalExceptionHandler`를 통해 해당 예외를 낚아채어 400 상태 코드와 통제된 에러 메시지만 반환하도록 예외 처리기를 고도화하고, 공격 시도를 모니터링할 수 있는 로깅 시스템을 구축했습니다.

### 🔒 [비즈니스 로직 무결성 및 DoS 방어] 트랜잭션 데이터 유실 제거 및 OOM 방어막 구축 (v2.9)

서버 전반의 도메인 연동 과정에서 발생한 논리적 사각지대를 박멸하고, 인메모리(RAM) 자원 고갈을 유발하는 악의적인 파일 업로드 공격 시나리오를 무력화했습니다.

* **⚡ 장바구니 이관 시 데이터 증발(Orphan Data) 결함 교정:**
    * **문제:** 장보기 완료 항목을 냉장고로 이관하는 `moveCheckedItemsToFridge` 로직 수행 시, 새로 생성된 `Ingredient` 엔티티에 소속 냉장고(`Fridge`) 매핑이 누락되어 데이터가 허공에 붕 뜨는 고아 데이터 생성 현상을 진단했습니다.
    * **해결:** 유저의 소속 냉장고 컨텍스트를 조회하여 생성 시점에 연관관계를 명확히 수립(`setFridge`)함으로써, 가족 공유 냉장고 도메인의 데이터 정합성과 영속성을 완벽히 확보했습니다.
* **🛡️ 대용량 파일 업로드 메모리 고갈(OOM/DoS) 원천 차단:**
    * **문제:** AI 이미지 인식(`MultipartFile`) API 호출 시 파일 업로드 용량 제한이 없어, 공격자가 거대한 더미 파일을 업로드할 경우 내부 톰캣 서버가 RAM 자원 임계치를 초과해 `OutOfMemoryError`로 다운되는 DoS 취약점을 식별했습니다.
    * **해결:** `application.properties` 내에 `max-file-size` 및 `max-request-size` 제한 설정을 10MB로 강제하여, 대용량 악성 파일이 힙 메모리에 적재되기 전 앞단에서 즉각 거부하도록 인프라 보안 수준을 강화했습니다.
